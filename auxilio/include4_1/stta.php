<?php
	...
	*/
	function displayTaches(){
		...
		$interval = $semaine1->diff($now);
		$intervalJours = $interval->format('%a');
		$semaine = (($intervalJours-($intervalJours%7))/7)+1;
		...
		$stmt = $db->prepare('SELECT * FROM stta WHERE (semaine <= :semaine AND status < 3) AND (user_id = :user_id) ORDER BY branche, delai');
	...
	function addTache($user_id, $value, $branche, $delai , $semaine){
		$db = loadDB(__FUNCTION__);
		
		$stmt = $db->prepare("INSERT INTO stta (user_id, value, branche, delai, semaine) VALUES (:user_id, :value, :branche, :delai, :semaine);");
	...
	function creeTacheSemaine($semaine){
		...
		for($i=0; $i<count($arrayOfTaches); $i++){
			...
			addTache($_SESSION['id'], ($arrayOfTaches[$i][0]), $arrayOfTaches[$i][1], $intermediaryDate->format('Y-m-d H:i:s'), $arrayOfTaches[$i][3]);
		}
		/*
		print '<pre>';
	...
	function updateStatusStta($semaine){
		...
		$stmt = $db->prepare('UPDATE stta SET status = 2 WHERE (status=0)  AND (semaine < :semaineActuel) AND user_id = :user_id');
		$stmt->bindParam(':semaineActuel', $semaine);
		$stmt->bindParam(':user_id', $_SESSION['id']);
		
		$stmt2 = $db->prepare('UPDATE stta SET status = 3 WHERE (status=1)  AND (semaine < :semaineActuel) AND user_id = :user_id');
