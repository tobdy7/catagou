<?php
	function addNote($noteValue){
		if($noteValue != '[DELETED]'){
			...
			$stmt = $db->prepare("INSERT INTO note (user_id, value) VALUES (:user_id, :value);");
	...
	function loadNotes(){
		...
		$stmt = $db->prepare('SELECT id, value FROM note WHERE displayNote = 1 AND user_id = :user_id');
		$stmt->bindParam(':user_id', $_SESSION['id']);
...
<script type="text/javascript">
		...
		function hideNote(id){
			...
			document.getElementById('note'+id).className = 'hidden';
