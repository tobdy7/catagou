	...
	function login($pseudo, $passwordHashed1){
		...
		if($result = $stmt->fetch()){
			if($result['access']>=1){
				...
				$return = '<script type="text/javascript">window.location = "http://localhost/auxilio/auxilio'.$_SESSION['version'].'.php";</script>';
			}else{
...
<div class="login" id="login">
	...
	<form method="POST" id="formLogin">
		<input type="text" id="loginPseudo" name="loginPseudo" onKeyUp="inputKeyUp(event, 'login');">
